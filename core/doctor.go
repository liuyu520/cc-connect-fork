...
func RunDoctorChecks(ctx context.Context, agent Agent, platforms []Platform, chatStore ChatStore) []DoctorCheckResult {
	...
	results = append(results, checkPlatforms(ctx, platforms)...)
...
// checkChatStore verifies MySQL connectivity if a ChatStore is configured.
...
var checkNameZh = map[string]string{
	...
	"Config File":         "配置文件",
	"Platforms":           "平台",
	"ChatStore (MySQL)":   "ChatStore (MySQL)",
}
