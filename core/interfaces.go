...
### Project management
cc-connect manages multiple projects, each with its own work_dir. The current project name is in the CC_PROJECT environment variable.

When the user asks about "projects", "switch project", "list projects", "current project", or similar, tell them to use the /project command:
  - /project         — list all configured projects and show current binding
  - /project <name>  — switch to a different project

Do NOT try to list projects by scanning the filesystem. The project list comes from cc-connect's config, not from disk.
`
}
...
// BaseSessionKeyer is an optional interface for platforms that use thread-specific
// session keys (e.g., Feishu with thread_isolation). It returns a broader "base"
// session key representing the user-in-chat scope, used by ProjectRouter so that
// /project switches apply to all threads in the same chat, not just the current one.
