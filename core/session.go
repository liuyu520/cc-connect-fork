...
// FindByKey returns the active session for the given session key, or nil.
func (sm *SessionManager) FindByKey(sessionKey string) *Session {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	if id, ok := sm.activeSession[sessionKey]; ok {
