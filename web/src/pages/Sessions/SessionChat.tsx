    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] animate-fade-in">
      ...
      <div className="flex-1 overflow-y-auto py-6 space-y-5">
        ...
        {session?.history?.map((msg, i) => {
          ...
          // 格式化消息时间：今天显示 HH:mm，今年显示 MM-DD HH:mm，更早显示完整日期
          const msgTime = (() => {
            ...
            if (date.getFullYear() === now.getFullYear() && date.getMonth() === now.getMonth() && date.getDate() === now.getDate()) return hhmm;
          ...
          return (
            <div key={i} className={cn('flex gap-3', isUser ? 'justify-end' : 'justify-start')}>
              ...
              <div className={cn('flex flex-col', isUser ? 'items-end' : 'items-start', 'gap-0.5')}>
                <div className={cn(
                ...
                {msgTime && (
                  <span className="text-[10px] text-gray-400 dark:text-gray-500 px-1">{msgTime}</span>
                )}
              </div>
              {isUser && (
