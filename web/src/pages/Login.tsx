...
export default function Login() {
  ...
  // 从 localStorage 恢复上次输入的值，方便用户再次登录
  const [token, setToken] = useState(() => localStorage.getItem('cc_token') || '');
  const [serverUrl, setServerUrl] = useState(() => localStorage.getItem('cc_server_url') || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(token.trim() ? t('login.invalidToken') : t('login.connectionFailed'));
      api.setToken('');
      api.setBaseUrl('');
    } finally {
      setLoading(false);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('login.token')} <span className="text-gray-400 font-normal">({t('common.optional')})</span>
              </label>
              <input
                type="password"
                value={token}
                onChange={(e) => setToken(e.target.value)}
                onBlur={(e) => { const v = e.target.value.trim(); v ? localStorage.setItem('cc_token', v) : localStorage.removeItem('cc_token'); }}
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onBlur={(e) => { const v = e.target.value.trim(); v ? localStorage.setItem('cc_server_url', v) : localStorage.removeItem('cc_server_url'); }}
