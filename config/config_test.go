...
func TestConfigValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg: func() Config {
				c := validBaseConfig()
				c.Projects = []ProjectConfig{validProject("")}
				return c
			}(),
...
func TestReadAndParse(t *testing.T) {
	...
	os.WriteFile(path, []byte(baseConfigTOML), 0o644)
...
// validBaseConfig 返回一个通过基础校验的 Config（含 database.dsn 和 management.port）
func validBaseConfig() Config {
	return Config{
		Database:   DatabaseConfig{DSN: "test:test@tcp(127.0.0.1:3306)/test?charset=utf8mb4&parseTime=true"},
...
func TestCloneAgentConfig(t *testing.T) {
	t.Run("without providers", func(t *testing.T) {
		in := AgentConfig{
			Type:    "codex",
			Options: map[string]any{"work_dir": "/tmp/codex"},
		}
		got := cloneAgentConfig(in)
		if got.Type != "codex" {
			t.Errorf("Type = %q, want codex", got.Type)
		}
		if got.Options["work_dir"] != "/tmp/codex" {
	...
	t.Run("with providers", func(t *testing.T) {
		...
		got.Options["work_dir"] = "changed"
		if in.Options["work_dir"] == "changed" {
